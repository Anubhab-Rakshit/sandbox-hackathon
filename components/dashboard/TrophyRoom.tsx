    ...
    return (
        <div className="w-full h-full bg-[#030303] overflow-y-auto p-8 border-l border-[#222]">
            <div className="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-[0.3em] text-white">THE TROPHY ROOM</h2>
            ...
            {trophies.length === 0 ? (
                <div className="text-[#333] tracking-widest text-xs uppercase h-64 border border-[#1a1a1a] flex flex-col items-center justify-center gap-4" style={{ background: 'repeating-linear-gradient(45deg, #020202, #020202 10px, #050505 10px, #050505 20px)' }}>
                    <div className="text-2xl opacity-20">🛡️</div>
                    <div className="font-mono">Awaiting Trophies - No advanced toolchains captured yet.</div>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {trophies.map((t) => (
                        <div key={t.threat_id} className="border border-[#1a1a1a] bg-black p-5 relative group hover:border-[#FFD700]/50 transition-colors shadow-2xl">
                            {/* Gold corner accents */}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="pt-2">
                                    <div className="text-[8px] text-[#555] tracking-widest uppercase mb-2">Fingerprint Sequence (Last 8)</div>
                                    <div className="flex flex-wrap gap-1.5">
                                        {t.payloads.slice(-8).map((p, idx) => (
                                            <span key={idx} className="text-[8px] font-mono px-1.5 py-0.5 bg-[#0a0a0a] text-[#00FFD1] border border-[#222]">
                                                {p.method}
                                            </span>
                                        ))}
                                        {t.payloads.length > 8 && (
